				...
				for i := 0; i < originalVal.NumField(); i++ {
					...
					if originalField.Kind() == reflect.Map || originalField.Kind() == reflect.Slice || originalField.Kind() == reflect.Pointer {
						...
						if (originalField.Len() > 0 || (originalField.Kind() == reflect.Pointer && !originalField.IsNil())) && checkField.IsZero() {
							// Original has data but user input is empty/zero, this is acceptablebecause YAML unmarshaling omits fields that aren't in the input
							continue
						}
						...
						if !originalField.IsZero() && !checkField.IsZero() {
							if !reflect.DeepEqual(originalField.Interface(), checkField.Interface()) {
								logger.Debugf("FIELD %s DIFFERS: Original: %+v, Check: %+v\n", fieldName, originalField.Interface(), checkField.Interface())
								isValid = false
								break
							}
						}
					...
					// For non-container types, use regular comparison but be lenient with zero values. If the user input has a zero value but original has non-zero, that's acceptable
			...
			if isSnapshot {
				...
				if err == nil {
					...
					for i := 0; i < originalVal.NumField(); i++ {
						...
						// For non-container types, use regular comparison but be lenient with zero values
						if !checkField.IsZero() && !reflect.DeepEqual(originalField.Interface(), checkField.Interface()) {
							logger.Debugf("FIELD %s DIFFERS: Original: %+v, Check: %+v\n", fieldName, originalField.Interface(), checkField.Interface())
							isValid = false
							break
